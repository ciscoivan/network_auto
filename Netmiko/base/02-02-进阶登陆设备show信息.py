...
if __name__ == '__main__':
    ...
    dev = {
        ...
        'timeout': 1.5  # 简单理解为执行一条命令并等待回显的时间，默认100，如果是show running或者interface 回显时间特别长的可以调大此参数
    }
    # 如果log想以某种规律命名，可以修正一下 f-string 或者format函数
    # dev['session_log'] = f"{dev['host']}.log"
    # dev['session_log'] = "{x}.log".format(x=dev['host'])

    # 调用send_command，基于pattern的命令执行，多用于show
    ...
    # # 调用send_command_timing，可以保证设备执行的命令后等回显等足timeout的时间，多用于show
    # with Netmiko(**dev) as net_conn:
    #     net_conn.enable()
    #     output = net_conn.send_command_timing('show running')
    #     print(output)

    # 使用send_command expect string参数
    with Netmiko(**dev) as net_conn:
    ...
    # # 使用 send_command_timing 做一些无法预料回显的
    # with Netmiko(**dev) as net_conn:
    #     # net_conn.enable()
    ...
    # 使用send_command expect string参数 配置端口
    # with Netmiko(**dev) as net_conn:
    #     prompt = net_conn.find_prompt()
    #
    #     hostname = get_dev_hostname(prompt)
